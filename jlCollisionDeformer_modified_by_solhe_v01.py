...
class jlCollisionDeformer(OpenMayaMPx.MPxDeformerNode):
    ...
    def compute(self,plug,dataBlock):
        ...
        if envelopeValue != 0:
            ...
            for k in range(self.newpoints.length() - 1):
            ...
            #write by Sol He.
            ###############################################################################
            #print Connected_vertex_list
            for k in range(0 , self.newpoints.length() - 1):
                ConnectIndecies = OpenMaya.MIntArray()
                Inmesh_vertex_iter.getConnectedVertices(ConnectIndecies)
                Inmesh_vertex_iter.next()

                if statuslist[k] == 1:
                    self.newpoints[k].x = self.tempArr[k].x / (ConnectIndecies.length () + 1)
                    self.newpoints[k].y = self.tempArr[k].y / (ConnectIndecies.length () + 1)
                    self.newpoints[k].z = self.tempArr[k].z / (ConnectIndecies.length () + 1)  
                    #print Connected_vertex_list[k]   
                    for m in ConnectIndecies:        
                        self.newpoints[k].x += (self.tempArr[m].x / (ConnectIndecies.length () + 1))
                        self.newpoints[k].y += (self.tempArr[m].y / (ConnectIndecies.length () + 1))
                        self.newpoints[k].z += (self.tempArr[m].z / (ConnectIndecies.length () + 1))
