...
maya.window(title = 'MatchMove',w=500,h=170)
...
commandstr = 'filename = cmds.fileDialog2(fileMode=1, caption="Import Image");\
...
maya.textField(targetsizeheight,tx = 'input target height' ,w=100,edit=True,enterCommand=('maya.setFocus(\"' + PixelAspectRatio + '\")'))

#complete the creating of pixel aspect ratio
maya.textField(PixelAspectRatio,tx = float(defaultpixelaspectratio),edit = True,enterCommand = ('maya.setFocus(\"' + sourcesizewidth + '\")'))
...
maya.button(label="Default Size",w=100,command = 'selected_node = maya.ls(sl=1);\
	...
	maya.setAttr("defaultResolution.pixelAspect",defaultpixelaspectratio);\
	')
...
maya.button(label="Go",w = 100,command = 'selected_node = maya.ls(sl=1);\
	...
	pixelAspectvalue = maya.textField(PixelAspectRatio,q = 1,tx = 1);\
