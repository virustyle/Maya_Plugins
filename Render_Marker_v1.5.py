...
def CreateNukeProject(seqpathList,camDataLsit,geopath,writepath,projpath,startframe,endframe,fps,W_size,H_size,scene_view,stereo,framecolor):
    ...
    camNode.setKnob('xpos'            ,11)
    camNode.setKnob('ypos'            ,107)
    nukeScriptWriter.addNode(camNode)
    

    #create the DirectLight node
    DirectLNode1 = Node('DirectLight')
    DirectLNode1.setKnob('inputs',0)
    DirectLNode1.setKnob('intensity','0.3')
    DirectLNode1.setKnob('rotate','{-40 270 0}')
    DirectLNode1.setKnob('depthmap_slope_bias','0.01')
    DirectLNode1.setKnob('xpos',11)
    DirectLNode1.setKnob('ypos',-262)
    nukeScriptWriter.addNode(DirectLNode1)

    DirectLNode2 = Node('DirectLight')
    DirectLNode2.setKnob('inputs',0)
    DirectLNode2.setKnob('intensity','0.3')
    DirectLNode2.setKnob('rotate','{-40 180 0}')
    DirectLNode2.setKnob('depthmap_slope_bias','0.01')
    DirectLNode2.setKnob('xpos',11)
    DirectLNode2.setKnob('ypos',-385)
    nukeScriptWriter.addNode(DirectLNode2)

    DirectLNode3 = Node('DirectLight')
    DirectLNode3.setKnob('inputs',0)
    DirectLNode3.setKnob('intensity','0.3')
    DirectLNode3.setKnob('rotate','{-40 0 0}')
    DirectLNode3.setKnob('depthmap_slope_bias','0.01')
    DirectLNode3.setKnob('xpos',11)
    DirectLNode3.setKnob('ypos',-325)
    nukeScriptWriter.addNode(DirectLNode3)

    DirectLNode4 = Node('DirectLight')
    DirectLNode4.setKnob('inputs',0)
    DirectLNode4.setKnob('intensity','0.3')
    DirectLNode4.setKnob('rotate','{-40 90 0}')
    DirectLNode4.setKnob('depthmap_slope_bias','0.01')
    DirectLNode4.setKnob('xpos',11)
    DirectLNode4.setKnob('ypos',-201)
    nukeScriptWriter.addNode(DirectLNode4)


    #create Constant Node
    ConstantNode1 = Node("Constant")
    ConstantNode1.setKnob('inputs',0)
    ConstantNode1.setKnob('channels','rgb')
    ConstantNode1.setKnob('color'   ,'{%s 1}'%framecolor)
    ConstantNode1.setKnob('format'  ,"%s %s 0 0 %s %s 1"%(W_size,H_size,W_size,H_size))
    ConstantNode1.setKnob('xpos',-292)
    ConstantNode1.setKnob('ypos',-336)
    nukeScriptWriter.addNode(ConstantNode1)


    #create Diffuse Node
    DiffuseNode = Node('Diffuse')
    DiffuseNode.setKnob('inputs',2)
    DiffuseNode.setKnob('white',0.25)
    DiffuseNode.setKnob('xpos',-202)
    DiffuseNode.setKnob('ypos',-336)
    nukeScriptWriter.addNode(DiffuseNode)
    ...
    GeoNode.setKnob('xpos'                , -202)
    GeoNode.setKnob('ypos'                , -261)
    nukeScriptWriter.addNode(GeoNode)


    #create Scene node
    SceneNode = Node('Scene')
    SceneNode.setKnob('inputs',5)
    SceneNode.setKnob('xpos',-192)
    SceneNode.setKnob('ypos',-201)
    nukeScriptWriter.addNode(SceneNode)
    ...
    readNode.setKnob('xpos' ,-292)
    readNode.setKnob('ypos' ,107)
...
class SampleData():
    ...
    def getAbcPath(self):
        ...
        if match:
            front = match.group(1)
            back  = match.group(2)
            self.abcpath = os.path.dirname(self.mayascript) + '/%s_marker%s.abc'%(front,back)
        else:
            self.abcpath = os.path.dirname(self.mayascript) + '/%s_marker.abc'%(projname)
        return self.abcpath
    ...
    def getFbxPath(self):
        ...
        if match:
            front = match.group(1)
            back  = match.group(2)
            self.fbxpath = os.path.dirname(self.mayascript) + '/%s_marker_camera%s.fbx'%(front,back)
        else:
            self.fbxpath = os.path.dirname(self.mayascript) + '/%s_marker_camera.fbx'%(projname)
        return self.fbxpath
    ...
    def getNukePath(self):
        ...
        if match:
            front = match.group(1)
            back  = match.group(2)
            self.nukepath = os.path.dirname(self.mayascript) + '/%s_marker%s.nk'%(front,back)
        else:
            self.nukepath = os.path.dirname(self.mayascript) + '/%s_marker.nk'%(projname)
    ...
    def getRenderPath(self,stereo = None):
        ...
        if match:
            ...
            back  = match.group(2)
            if stereo == True:
                self.renderpath = os.path.dirname(self.mayascript) + '/%s_marker%s/%s_marker%s.####.jpg'%(front,r"_%V" + back,front,r"_%V" + back)
            else:
                self.renderpath = os.path.dirname(self.mayascript) + '/%s_marker%s/%s_marker%s.####.jpg'%(front,back,front,back)
        else:
            if stereo == True:
                self.renderpath = os.path.dirname(self.mayascript) + '/%s_marker%s/%s_marker_%s.####.jpg'%(projname,r"%V",projname,r"%V")
            else:
                self.renderpath = os.path.dirname(self.mayascript) + '/%s_marker/%s_marker.####.jpg'%(projname,projname)
...
class MainWindows(QtGui.QWidget):
    ...
    def __init__(self,parent = None):
        ...
        logoLabel      = QtGui.QLabel("<font size='12' color='gray'><B>RENDER MARKER TOOL</B></font>")
        ...
        self.setWindowTitle("RENDER MARKER TOOL v1.5")
