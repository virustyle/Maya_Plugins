...
def CreateNukeProject(seqpathList,camDataLsit,geopath,writepath,projpath,startframe,endframe,fps,W_size,H_size,scene_view,stereo):
...
class SampleData():
    ...
    def getRenderPath(self,stereo = None):
        ...
        if match:
            ...
            back  = match.group(2)
            if stereo == True:
                self.renderpath = os.path.dirname(self.mayascript) + '/%s_wire%s/%s_wire%s.####.jpg'%(front,back,front,r"_%V" + back)
            else:
                self.renderpath = os.path.dirname(self.mayascript) + '/%s_wire%s/%s_wire%s.####.jpg'%(front,back,front,back)
        else:
            if stereo == True:
                self.renderpath = os.path.dirname(self.mayascript) + '/%s_wire/%s_wire_%s.####.jpg'%(projname,projname,r"%V")
            else:
...
class MainWindows(QtGui.QWidget):
    ...
    def __init__(self,parent = None):
        ...
        self.setWindowTitle("RENDER WIREFRAME TOOL v1.3")
    ...
    def submitRender(self):
        ...
        CreateNukeProject(seqpathList,camDataLsit,geopath,writepath,projpath,startframe,endframe,fps,W_size,H_size,scene_view,stereo)
