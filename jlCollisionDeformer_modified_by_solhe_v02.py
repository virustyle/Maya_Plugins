...
class jlCollisionDeformer(OpenMayaMPx.MPxDeformerNode):
    ...
    def compute(self,plug,dataBlock):
        ...
        Inmesh_vertex_iter_sub = OpenMaya.MItMeshVertex(newObject)
        ...
        #print self.colliderBBoxSize
        #print colliderBBSizeValue
        #print colliderBBVector
        #print colliderBBSize
        ...
        if envelopeValue != 0:
            ...
            statuslist = [0 for i in range(vertexcount)]
            ...
            for k in range( vertexcount ):
                ...
                if gotHit == True:
                    ...
                    if collision == 1:
                        ...
                        hitPointDirection = OpenMaya.MFloatVector(pointNormal)
                        ...
                        #angle = closePointNormal*rayDirection #closePointNormal*rayDirection


                        if bulgeValue >= 0:
                            worldPoint = OpenMaya.MPoint(hitPoints1[signChange])
                        else:
                            worldPoint = OpenMaya.MPoint(hitPoints1[signChange + 1])
            ...
            if checkCollision != 0:
                ...
                for i in range(vertexcount):
                    ...
                    if statuslist[i] == 1:                    
                        Inmesh_vertex_iter_sub.getConnectedVertices(ConnectIndecies)
                        #print ConnectIndecies
                        self.newpoints[i].x = self.tempArr[i].x / (ConnectIndecies.length () + 1)
                        self.newpoints[i].y = self.tempArr[i].y / (ConnectIndecies.length () + 1)
                        self.newpoints[i].z = self.tempArr[i].z / (ConnectIndecies.length () + 1)  
                    
                        for m in ConnectIndecies:        
                            self.newpoints[i].x += (self.tempArr[m].x / (ConnectIndecies.length () + 1))
                            self.newpoints[i].y += (self.tempArr[m].y / (ConnectIndecies.length () + 1))
                            self.newpoints[i].z += (self.tempArr[m].z / (ConnectIndecies.length () + 1))
                    else:
                        self.newpoints[i].x = self.tempArr[i].x
                        self.newpoints[i].y = self.tempArr[i].y
                        self.newpoints[i].z = self.tempArr[i].z

                    Inmesh_vertex_iter_sub.next()

                    if bulgeExtendValue != 0:
                    #if False:
