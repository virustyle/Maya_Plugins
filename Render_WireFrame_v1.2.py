...
'''
  ...
  def serializeNode(self, nodeType, knobValues, userKnobs, order):
    ...
    if nodeType == 'ReadGeo2':
      self.readgeoendSerializeNode()
    else:
...
class ReadNodeKnobFormatter:
  def serialize(self, serializer, nodeType, knobValues, userKnobs, order):
    ...
    serializer.readendSerializeNode()
...
def CreateNukeProject(seqpath,campath,geopath,writepath,projpath,startframe,endframe,fps,W_size,H_size,scene_view):
    ...
    #camNode.setKnob('use_frame_rate'  ,'true')
    #camNode.setKnob('compute_rotation','true')
    camNode.setKnob('fbx_node_name'   ,'{{7}}')
    ...
    MergeNode.setKnob('operation','screen')
    MergeNode.setKnob('mix',0.7)
    MergeNode.setKnob('xpos',239)
    MergeNode.setKnob('ypos',215)
...
class MainWindows(QtGui.QWidget):
    ...
    def __init__(self,parent = None):
        ...
        self.camComb.addItems(self.mayadata.getCameralist())


        firstgridlayout = QtGui.QVBoxLayout()
        CameraLayout    = QtGui.QGridLayout()
        CameraLayout.addWidget( self.camLabel , 1 , 1 )
        CameraLayout.addWidget( self.camComb  , 1 , 2 , 1 , 3 )
        firstgridlayout.addLayout( CameraLayout )
        TimeLayout      = QtGui.QHBoxLayout()
        TimeLayout.addWidget( self.FFLabel )
        TimeLayout.addWidget( self.FFText )
        TimeLayout.addWidget( self.LFLabel )
        TimeLayout.addWidget( self.LFText )
        TimeLayout.addWidget( self.FPSLabel )
        TimeLayout.addWidget( self.FPSText )     
        firstgridlayout.addLayout(TimeLayout )
        self.frame2 = QtGui.QFrame(self)
        self.frame2.setLayout(firstgridlayout)
        ...
        self.renderpathText.setText( self.mayadata.getRenderPath() )
        ...
        self.frame3.setGeometry(QtCore.QRect(5, 215, 390, 180))
        ...
        print CameraLayout.cellRect(1,2),CameraLayout.cellRect(1,1)
        #Set the layout of QWidget.
        #Globallayout.setVerticalSpacing(10)
        #self.setLayout( Globallayout )
        self.setWindowTitle("RENDER WIREFRAME TOOL v1.2")
    ...
    def exportAbcFile(self):
        ...
        #print self.nuke_camera_scene_view_imported
        #print self.nuke_camera_scene_view_selected
        #print self.nuke_camera_scene_view_items
        ...
        #print self.nuke_camera_scene_view
    ...
    def submitRender(self):
        ...
        imageplane = cmds.listConnections( cmds.listRelatives( self.camComb.currentText() , shapes = True )[0] , connections = True , type = 'imagePlane')[-1]

        #Create nuke script
        seqpath    = '.'.join(cmds.getAttr('%s.imageName'%imageplane).split('.')[:-2]) + '.####.' + cmds.getAttr('%s.imageName'%imageplane).split('.')[-1]
        campath    = self.mayadata.getFbxPath()
        ...
        CreateNukeProject(seqpath,campath,geopath,writepath,projpath,startframe,endframe,fps,W_size,H_size,scene_view)
